...
func removeBudgetByCookie(w http.ResponseWriter, r *http.Request) {
	setupResponse(&w, r)
	var budget models.Budget
	...
	if cookie == "" {
		...
		err = budgetCollection.FindOneAndDelete(context.TODO(), bson.M{"budget_id": budget.Budget_id, "user_id": user.User_id}).Decode(&budget)
		if err != nil {
			fmt.Println(err)
			w.WriteHeader(http.StatusBadRequest)
			return
